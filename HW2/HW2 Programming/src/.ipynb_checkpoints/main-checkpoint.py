...
async def get_students(req: Request):
	...
	if 'fields' in params.keys():
		fields_formatted = list(params['fields'].split(','))
		del params['fields']
		json_output = jsonable_encoder(db.select("student", fields_formatted, filters=params))
		return JSONResponse(content=json_output, status_code=status.HTTP_200_OK)

	else:
		json_output = jsonable_encoder(db.select("student", [], filters=params))
		return JSONResponse(content=json_output, status_code=status.HTTP_200_OK)
...
async def get_student(student_id: int):
	...
	query=db.select("student", [], filters)
	if len(query) == 0:
		return HTMLResponse(status_code=status.HTTP_404_NOT_FOUND)

	else:
		json_output = jsonable_encoder(query[0])
		return JSONResponse(content=json_output, status_code=status.HTTP_200_OK)
...
async def put_student(student_id: int, req: Request):
	...
	#Note: I assume that updating a student to its current values should be allowed with HTTP status 200 Ok
	query = db.select("student", [], filters)
	if len(query) == 0:
		return HTMLResponse(status_code=status.HTTP_404_NOT_FOUND)
	else:
		try:
			db.update('student', params,filters)
		except:
			return HTMLResponse(status_code=status.HTTP_400_BAD_REQUEST)
		else:
...
async def get_employees(req: Request):
	...
	if 'fields' in params.keys():
		fields_formatted = list(params['fields'].split(','))
		del params['fields']
		json_output = jsonable_encoder(db.select("employee", fields_formatted, filters=params))
		return JSONResponse(content=json_output, status_code=status.HTTP_200_OK)

	else:
		json_output = jsonable_encoder(db.select("employee", [], filters=params))
		return JSONResponse(content=json_output, status_code=status.HTTP_200_OK)
...
async def get_employee(employee_id: int):
	...
	query = db.select("employee", [], filters)
	if len(query) == 0:
		return HTMLResponse(status_code=status.HTTP_404_NOT_FOUND)
...
async def put_employee(employee_id: int, req: Request):
	...
	if len(query) == 0:
		...
		try:
			db.update('employee', params, filters)
		except:
			return HTMLResponse(status_code=status.HTTP_400_BAD_REQUEST)
		else:
			return HTMLResponse(status_code=status.HTTP_200_OK)
